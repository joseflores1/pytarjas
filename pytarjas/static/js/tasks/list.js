...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Parse form definitions from the hidden textarea
    ...
    function updateDynamicFilters() {
        ...
        // Clear old dynamic filters
        document.querySelectorAll('.dynamic-filter-group').forEach(el => {
            el.remove();
        });
        
        if (!selectedFormId || !allFormsData[selectedFormId]) {
            return;
        }
        ...
        form.questions.forEach(q => {
            ...
            // Only show filters for non-file/photo questions
            if (['text', 'textarea', 'number', 'select', 'client_select'].includes(type)) {
                ...
                // Use original question text as label
                const labelText = q.text.replace(/[\*\?]/g, '').trim(); 
    ...
    // Initialize dynamic filters on load
    if (formIdSelect) {
    ...
    document.querySelectorAll('.custom-filter-display').forEach(display => {
        display.addEventListener('click', function() {
            ...
            // Reset categories to collapsed state
            menu.querySelectorAll('.filter-user-list').forEach(l => {
                l.style.display = 'none';
            });
            menu.querySelectorAll('.filter-role-category').forEach(h => {
    ...
    // --- 4. DEBOUNCED TRIGGER & URL UPDATE ---
    let debounceTimer;
    ...
    // Stop refresh if user is interacting with the form
    if (filterForm) {
